...
enum BindTargets { BindTargets_PS, BindTargets_VS };
...
void RSetPrimitiveTopology(struct Renderer *renderer,
			   D3D11_PRIMITIVE_TOPOLOGY topology);

void RSetInputLayout(struct Renderer *renderer, ID3D11InputLayout *inputLayout);

void RSetRasterizerState(struct Renderer *renderer,
			 ID3D11RasterizerState *rasterizerState);
...
void RBindShaderResources(struct Renderer *renderer,
			  enum BindTargets bindTarget,
			  ID3D11ShaderResourceView **SRVs, uint32_t numSRVs);

void RBindConstantBuffers(struct Renderer *renderer,
			  enum BindTargets bindTarget, ID3D11Buffer **CBs,
			  uint32_t numCBs);

void RDrawIndexed(struct Renderer *renderer, ID3D11Buffer *indexBuffer,
		  ID3D11Buffer *vertexBuffer, uint32_t strides,
		  uint32_t indexCount, uint32_t startIndexLocation,
