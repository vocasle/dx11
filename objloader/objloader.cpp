...
static void OLParseMeshes(std::vector<Mesh> &meshes,
			  std::vector<MeshInfo> &infos, FILE *objfile)
{
	char line[128];
	char prefix[4];
	float vec[3];
	...
	while (OLReadLine(objfile, line, 128)) {
		...
		if (strcmp(prefix, "o ") == 0) {
			...
			OLLogInfo("Normal { %f %f %f }", vec[0], vec[1],
				  vec[2]);
		} else if (strcmp(prefix, "f ") == 0) {
			result = sscanf_s(line, "%s%d/%d/%d%d/%d/%d%d/%d/%d",
					  prefix, 4, idx, idx + 1, idx + 2,
					  idx + 3, idx + 4, idx + 5, idx + 6,
					  idx + 7, idx + 8);
			assert(result == 10);
			OLLogInfo("Face { %d/%d/%d %d/%d/%d %d/%d/%d }", idx[0],
...
static std::string OLGetCwd(const char *filename)
{
	...
	for (uint32_t i = 0; i < model->Meshes.size(); ++i) {
		...
		for (uint32_t j = 0; j < m->Positions.size(); ++j) {
			fprintf(f, "Position { %f %f %f }\n", m->Positions[j].X,
				m->Positions[j].Y, m->Positions[j].Z);
		}
