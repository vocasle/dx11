...
void CubeMap::LoadCubeMap(ID3D11Device *device,
			  const std::vector<const char *> &filepaths)
{
	...
	uint8_t *bytes = stbi_load(filepaths[0], &width, &height,
	...
	for (uint32_t i = 0; i < filepaths.size(); ++i) {
		pBytes[i] = stbi_load(filepaths[i], &width, &height,
				      &channelsInFile, desiredChannels);
		...
		if (!pBytes[i]) {
			UtilsDebugPrint(
				"ERROR: Failed to load texture from %s\n",
				filepaths[i]);
	...
	HR(device->CreateTexture2D(&texDesc, pData,
				   texture.ReleaseAndGetAddressOf()));
	HR(device->CreateShaderResourceView(texture.Get(), &SMViewDesc,
...
void CubeMap::CreateCube(const Actor &actor, ID3D11Device *device)
{
	...
	m_indices.insert(std::end(m_indices), std::begin(actor.GetIndices()),
