...
void Renderer::SetVertexBuffer(ID3D11Buffer *buffer, uint32_t strides,
			       uint32_t offsets)
{
...
void Renderer::SetRenderTargets(ID3D11RenderTargetView *rtv,
...
void Renderer::SetBlendState(ID3D11BlendState *blendState)
{
	...
	m_DR->GetDeviceContext()->OMSetBlendState(blendState, nullptr,
						  sampleMask);
}

void Renderer::BindShaderResources(enum BindTargets bindTarget,
...
void Renderer::BindConstantBuffers(enum BindTargets bindTarget,
				   ID3D11Buffer **CBs, uint32_t numCBs)
{
...
void Renderer::BindShaderResource(enum BindTargets bindTarget,
...
void Renderer::BindConstantBuffer(enum BindTargets bindTarget, ID3D11Buffer *cb,
				  uint32_t slot)
{
...
void Renderer::DrawIndexed(uint32_t indexCount, uint32_t startIndexLocation,
...
void Renderer::ClearRenderTargetView(ID3D11RenderTargetView *rtv,
				     const float *color)
{
	m_DR->GetDeviceContext()->ClearRenderTargetView(rtv, color);
}

void Renderer::ClearDepthStencilView(ID3D11DepthStencilView *dsv, uint32_t mask,
				     float depth, uint8_t stencil)
{
	m_DR->GetDeviceContext()->ClearDepthStencilView(dsv, mask, depth,
							stencil);
}
