...
Actor::Actor ()
    : m_IndexBuffer{ nullptr }, m_VertexBuffer{ nullptr },
      ...
      m_Rotation{ MathMat4X4Identity () }, m_Scale{ MathMat4X4Identity () },
      m_Translation{ MathMat4X4Identity () }, m_DiffuseTexture{ nullptr },
      m_SpecularTexture{ nullptr }, m_GlossTexture{ nullptr },
      m_NormalTexture{ nullptr }, m_Material{}, m_Name{}, m_IsVisible{ true }
{
}
  ...
  for (long a = 0; a < m_Vertices.size (); a++)
    {
      ...
      m_Vertices[a].Bitangent
          = MathVec3DCross (&m_Vertices[a].Normal, &m_Vertices[a].Tangent);
...
{
  UtilsCreateVertexBuffer (device, &m_Vertices[0], m_Vertices.size (),
                           sizeof (Vertex),
...
{
  UtilsCreateIndexBuffer (device, &m_Indices[0], m_Indices.size (),
                    ...
                    ID3D11Device *device, ID3D11DeviceContext *context)
{
  ...
  Microsoft::WRL::ComPtr<ID3D11Texture2D> texture;
  {
    ...
    context->UpdateSubresource (texture.Get (), 0, nullptr, bytes,
                                width * sizeof (uint8_t) * desiredChannels, 0);
  }
