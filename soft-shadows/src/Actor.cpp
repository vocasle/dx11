...
void Actor::GenerateTangents()
{
	...
	for (long a = 0; a < m_Vertices.size(); a++) {
		...
		m_Vertices[a].Bitangent = MathVec3DCross(
			&m_Vertices[a].Normal, &m_Vertices[a].Tangent);
...
void Actor::LoadTexture(const char *filename, enum TextureType type,
			ID3D11Device *device, ID3D11DeviceContext *context)
{
	...
	unsigned char *bytes = stbi_load(filename, &width, &height,
