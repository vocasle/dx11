...
Game::UpdateImgui() {
    ...
    if (ImGui::CollapsingHeader("Rasterizer settings")) {
        ...
        ImGui::SliderInt("CullMode",
                         ...
                         1,
                         3);
        ImGui::InputFloat("DepthBiasClamp", &g_rasterizerDesc.DepthBiasClamp);
        ImGui::InputFloat("SlopeScaledDepthBias",
        ...
        ImGui::Checkbox(
            "ScissorEnable",
            reinterpret_cast<bool *>(&g_rasterizerDesc.ScissorEnable));
        ImGui::Checkbox(
            "MultisampleEnable",
        ...
        ImGui::PopItemWidth();
        if (ImGui::Button("Apply##Rasterizer")) {
            ...
            if (errorBlob.Get()) {
                ...
                if (sn.find("VS") != std::string::npos) {
                    if (m_shaderManager.GetVertexShader(sn)) {
                        ...
                        const HRESULT hr =
                            m_DR->GetDevice()->CreateVertexShader(
...
Game::Render() {
    ...
    m_DR->PIXEndEvent();
    //// TODO: Need to have a reflection mechanism to query amount of SRV that
    /// is possible to bind to PS / After this this clear code could be placed
    /// to
    /// Renderer::Clear
