...
void Game::UpdateImgui()
{
...

	{
		...
		if (ImGui::Button("Open")) {
			if (shaderName.find(".hlsl") != std::string::npos) {
				const std::string shaderPath =
					UtilsFormatStr("%s/shader/%s", SRC_ROOT,
						       shaderName.c_str());
				const auto bytes =
					UtilsReadData(shaderPath.c_str());
		...
		if (ImGui::Button("Compile")) {
			...
			UtilsWriteData(shaderPath.c_str(), &buffer[0], sz,
				       true);
			// compile shader
			const std::string sn =
			...
			std::mbstowcs(&shaderPathW[0], &shaderPath[0],
				      shaderPathW.size());
			...
			if (errorBlob.Get()) {
				...
				if (sn.find("VS") != std::string::npos) {
					if (m_shaderManager.GetVertexShader(
						    sn)) {
					...
					if (m_shaderManager.GetPixelShader(
						    sn)) {
						...
						UtilsDebugPrint(
							...
							shaderName.c_str());
						const HRESULT hr =
							m_DR->GetDevice()->CreatePixelShader(
								shaderBlob
									->GetBufferPointer(),
								shaderBlob
									->GetBufferSize(),
