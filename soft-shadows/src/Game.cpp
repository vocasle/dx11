...
{
  auto it = std::find_if (
      std::begin (m_Actors), std::end (m_Actors),
      [&name] (const Actor &actor) { return actor.GetName () == name; });
...
Game::UpdateImgui ()
{
  // Any application code here
  {
    ...
    ImGui::InputFloat3 (
        "Dir light pos",
        reinterpret_cast<float *> (&m_PerSceneData.dirLight.Position));
  ...
  if (ImGui::CollapsingHeader ("Rasterizer settings"))
    {
      ImGui::PushItemWidth (150.0f);
      ImGui::SliderInt ("FillMode",
                        reinterpret_cast<int *> (&g_rasterizerDesc.FillMode),
                        2, 3);
      ImGui::Checkbox (
          "FrontCounterClockwise",
          reinterpret_cast<bool *> (&g_rasterizerDesc.FrontCounterClockwise));
      ImGui::SliderInt ("DepthBias", &g_rasterizerDesc.DepthBias, 0, 10000);
      ImGui::SliderInt ("CullMode",
                        reinterpret_cast<int *> (&g_rasterizerDesc.CullMode),
                        1, 3);
      ImGui::SliderFloat ("DepthBiasClamp", &g_rasterizerDesc.DepthBiasClamp,
                          -1000.0f, 1000.0f);
      ImGui::SliderFloat ("SlopeScaledDepthBias",
                          &g_rasterizerDesc.SlopeScaledDepthBias, -1000.0f,
                          1000.0f);
      ImGui::Checkbox (
          "DepthClipEnable",
          reinterpret_cast<bool *> (&g_rasterizerDesc.DepthClipEnable));
      ImGui::Checkbox ("ScissorEnable", reinterpret_cast<bool *> (
                                            &g_rasterizerDesc.ScissorEnable));
      ImGui::Checkbox (
          "MultisampleEnable",
          reinterpret_cast<bool *> (&g_rasterizerDesc.MultisampleEnable));
      ImGui::Checkbox (
          "AntialiasedLineEnable",
          reinterpret_cast<bool *> (&g_rasterizerDesc.AntialiasedLineEnable));
      ImGui::PopItemWidth ();
...

  {
    ...
    if (ImGui::Button ("Open"))
      {
        if (shaderName.find (".hlsl") != std::string::npos)
          {
            const std::string shaderPath = UtilsFormatStr (
                "%s/shader/%s", SRC_ROOT, shaderName.c_str ());
    ...
    if (ImGui::Button ("Compile"))
      {
        ...
        const std::string shaderPath
            = UtilsFormatStr ("%s/shader/%s", SRC_ROOT, shaderName.c_str ());
        ...
        // compile shader
        const std::string sn
            = shaderName.substr (0, shaderName.find (".hlsl"));
        ...
        HR (D3DCompileFromFile (
            shaderPathW.c_str (), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE,
            "main", isVS ? "vs_5_0" : "ps_5_0", 0, 0, &shaderBlob, &errorBlob))
                ...
                if (m_shaderManager.GetVertexShader (sn))
                  {
                    ...
                    const HRESULT hr = m_DR->GetDevice ()->CreateVertexShader (
                ...
                if (m_shaderManager.GetPixelShader (sn))
                  {
                    ...
                    UtilsDebugPrint ("Hot reload shader %s\n",
                                     shaderName.c_str ());
                    const HRESULT hr = m_DR->GetDevice ()->CreatePixelShader (
                        shaderBlob->GetBufferPointer (),
                        shaderBlob->GetBufferSize (), nullptr,
                        ps.GetAddressOf ());
                    UtilsDebugPrint ("Hot reloading shader %s. Result: %ld\n",
    ...
    if (ImGui::CollapsingHeader ("Shader Source"))
      {
        ImGui::InputTextMultiline ("Source", &buffer[0], buffer.size (),
                                   { 640.0f, 480.0f },
      ...
      if (ImGui::CollapsingHeader (UtilsFormatStr ("Cube %d", i).c_str ()))
        {
          ...
          ImGui::SliderAngle (UtilsFormatStr ("Cube %d Pitch", i).c_str (),
                              &cubeRotations[i].X);
          ImGui::SameLine ();
          ImGui::SliderAngle (UtilsFormatStr ("Cube %d Yaw", i).c_str (),
                              &cubeRotations[i].Y);
          ImGui::SameLine ();
          ImGui::SliderAngle (UtilsFormatStr ("Cube %d Roll", i).c_str (),
                              &cubeRotations[i].Z);
          ImGui::PopItemWidth ();
        }
    }
...
Game::Render ()
{
  ...
  ///Renderer::Clear
  // ID3D11ShaderResourceView* nullSRVs[7] = { nullptr, nullptr, nullptr,
...
Game::Initialize (HWND hWnd, uint32_t width, uint32_t height)
{
  ...
  const std::string droidSansTtf
      = UtilsFormatStr ("%s/../imgui-1.87/misc/fonts/DroidSans.ttf", SRC_ROOT);
...
void
Game::DrawActor (const Actor &actor)
{
  if (actor.IsVisible ())
    {
