	...
	const void *shaderData, const size_t sz, ID3D11Device *pD3DDevice)
{
	...
	const HRESULT hr = pD3DDevice->CreateInputLayout(
		...
		static_cast<uint32_t>(m_inputDescriptions.size()), shaderData,
		sz, m_inputLayout.ReleaseAndGetAddressOf());
...
InputLayout::InputLayout(ID3D11Device *device, const void *vsBytes,
			 const size_t sz)
	: m_strides(0){ HR(CreateInputLayoutDescFromVertexShaderSignature(
		  vsBytes, sz, device)) }
