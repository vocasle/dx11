...
InputLayout::CreateInputLayoutDescFromVertexShaderSignature (
    const void *shaderData, const size_t sz, ID3D11Device *pD3DDevice)
{
  // Reflect shader info
  ID3D11ShaderReflection *pVertexShaderReflection = nullptr;
  if (FAILED (
          D3DReflect (shaderData, sz, IID_ID3D11ShaderReflection,
  ...
  const HRESULT hr = pD3DDevice->CreateInputLayout (
      ...
      static_cast<uint32_t> (m_inputDescriptions.size ()), shaderData, sz,
...
InputLayout::InputLayout (ID3D11Device *device, const void *vsBytes,
                          const size_t sz)
    : m_strides (0){ HR (
        CreateInputLayoutDescFromVertexShaderSignature (vsBytes, sz, device)) }
