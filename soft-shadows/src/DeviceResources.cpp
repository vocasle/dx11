...
DeviceResources::DeviceResources ()
    : m_BackbufferWidth{ 0 }, m_BackbufferHeight{ 0 },
      m_MultiSampleQualityLevel{ 0 }, m_ScreenViewport{}, m_hWnd{ nullptr },
      m_OutputSize{}
{
...
DeviceResources::CreateFactory ()
{
...
#if _DEBUG
  {
    ...
    if (SUCCEEDED (DXGIGetDebugInterface1 (
            0, IID_PPV_ARGS (infoQueue.ReleaseAndGetAddressOf ()))))
      {
...
DeviceResources::CreateRasterizerState ()
{
  ...
  if (FAILED (m_Device->CreateRasterizerState (
          &rasterizerDesc, m_RasterizerState.ReleaseAndGetAddressOf ())))
    {
...
DeviceResources::CreateDeviceResources ()
{
  ...
  if (FAILED (D3D11CreateDevice (
          adapter.Get (), D3D_DRIVER_TYPE_UNKNOWN, NULL, createDeviceFlag,
...
DeviceResources::CreateWindowSizeDependentResources ()
{
  if (!m_hWnd)
    {
      UtilsDebugPrint ("ERROR: hWnd is not set! Windows size dependent "
  ...
  if (FAILED (m_Device->CheckMultisampleQualityLevels (
          DXGI_FORMAT_B8G8R8A8_UNORM, 4, &numQualityLevels)))
    {
      OutputDebugStringA (
          "ERROR: Failed to query multisample quality levels\n");
      ExitProcess (EXIT_FAILURE);
    }
  ...
  if (m_SwapChain)
    {
      if (FAILED (m_SwapChain->ResizeBuffers (
              2, backBufferWidth, backBufferHeight, backBufferFormat, 0)))
        {
  ...
  else
    {
      ...
      if (FAILED (m_Factory->CreateSwapChainForHwnd (
              m_Device.Get (), m_hWnd, &swapChainDesc, &fsSwapChainDesc, NULL,
      ...
      if (FAILED (m_Factory->MakeWindowAssociation (m_hWnd,
...
DeviceResources::ReportLiveObjects (void)
{
#ifdef _DEBUG
  {
    ...
    if (SUCCEEDED (DXGIGetDebugInterface1 (
            0, IID_PPV_ARGS (dxgiDebug.ReleaseAndGetAddressOf ()))))
      {
