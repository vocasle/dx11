...
ParticleSystem::ParticleSystem (const std::string &name, const Vec3D &origin,
    ...
    : m_name{ name }, m_vertices{}, m_indices{}, m_origin{ origin },
      m_camera{ &camera }, m_emitter{ ParticleType::Emitter, {}, {}, {}, 0.0f }
{
  ...
  for (Particle &p : m_particles)
    {
      if (p.IsAlive ())
        {
          p.Tick (deltaTime);
          p.CreateQuad (PARTICLE_SIZE, PARTICLE_SIZE, m_camera->GetUp (),
                        m_camera->GetRight ());
        }
      else
        {
...
Particle::Particle (ParticleType type, const Vec3D &accel,
    ...
    : m_accel{ accel }, m_initVel{ initVel }, m_initPos{ initPos }, m_pos{},
      m_lifespan{ lifespan }, m_type{ type }
{
}

void
Particle::CreateQuad (int width, int height, const Vec3D &up,
                      const Vec3D &right)
{
  Vec3D R = right;
