    ...
    : mWidth (width), mHeight (height), mFormat (format)
{
  ...
  HR (device->CreateShaderResourceView (mTexture.Get (), &srvDesc,
  ...
  HR (device->CreateRenderTargetView (mTexture.Get (), &rtvDesc,
                                      mRTV.ReleaseAndGetAddressOf ()))

  {
    ...
    HR (device->CreateTexture2D (&texDesc, NULL,
                                 mDepthTexture.ReleaseAndGetAddressOf ()))

    D3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc = {};
    dsvDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;
    dsvDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;

    HR (device->CreateDepthStencilView (mDepthTexture.Get (), &dsvDesc,
                                        mDSV.ReleaseAndGetAddressOf ()))
    ...
    HR (device->CreateShaderResourceView (mDepthTexture.Get (), &srvDesc,
                                          mDepthSRV.ReleaseAndGetAddressOf ()))
  }
}
