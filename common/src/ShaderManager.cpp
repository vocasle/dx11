...
{
	return m_activeVS.empty() ? 0 :
				    m_inputLayouts.at(m_activeVS).GetStrides();
}

ID3D11InputLayout *ShaderManager::GetInputLayout() const
{
	return m_activeVS.empty() ? nullptr :
				    m_inputLayouts.at(m_activeVS).Get();
}
...
void ShaderManager::UpdateVertexShader(const std::string &name,
				       ID3D11VertexShader *shader)
{
	m_vertexShaders[name] = shader;
}

void ShaderManager::UpdatePixelShader(const std::string &name,
				      ID3D11PixelShader *shader)
{
	m_pixelShaders[name] = shader;
}

void ShaderManager::CreateVertexShader(const std::string &filepath,
				       ID3D11Device *device)
{
	const auto bytes = UtilsReadData(
		UtilsFormatStr("%s/%s", m_shaderBlobRootDir.c_str(),
			       filepath.c_str())
			.c_str());
...
void ShaderManager::CreatePixelShader(const std::string &filepath,
				      ID3D11Device *device)
{
	const auto bytes = UtilsReadData(
		UtilsFormatStr("%s/%s", m_shaderBlobRootDir.c_str(),
...
void ShaderManager::Recompile(ID3D11Device *device)
{
	...
	do {
		...
		const std::string shaderNameStr =
			UtilsFormatStr("%s/%s", m_shaderSrcRootDir.c_str(),
