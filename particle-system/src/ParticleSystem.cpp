...
void ParticleSystem::Tick(const float deltaTime)
{
	...
	for (Particle &p : m_particles) {
		if (p.IsAlive()) {
			p.Tick(deltaTime);
			p.CreateQuad(m_particleSize.X, m_particleSize.Y,
...
void ParticleSystem::CreateTexture(ID3D11Device *device,
				   ...
				   const std::string &filepath)
{
	...
	ComPtr<ID3D11Texture2D> texture;
	{
		...
		context->UpdateSubresource(texture.Get(), 0, nullptr,
					   image.GetBytes(),
					   image.GetWidth() * sizeof(uint8_t) *
						   Image::DESIRED_CHANNELS,
					   0);
	}
...
void ParticleSystem::CreateBlendState(ID3D11Device *device)
{
	...
	HR(device->CreateBlendState(&desc,
...
void ParticleSystem::UpdateVertices()
{
	...
	for (const Particle &p : m_particles) {
		m_vertices.insert(m_vertices.end(), p.GetVertices().begin(),
				  p.GetVertices().end());
		for (int i = 0; i < 6; ++i) {
...
{
	Vec3D accel = m_accel;
	accel.X += MathRandom(-m_randomFactor, m_randomFactor);
	accel.Y += MathRandom(-m_randomFactor, m_randomFactor);
	accel.Z += MathRandom(-m_randomFactor, m_randomFactor);
	...
	Particle p = {
		ParticleType::Particle, accel, initVel, m_emitter.GetInitPos(), 0.0f, *this
	};
	p.CreateQuad(m_particleSize.X, m_particleSize.Y, m_camera->GetUp(),
		     m_camera->GetRight());
	m_particles.push_back(p);
	m_vertices.insert(m_vertices.end(), p.GetVertices().begin(),
			  p.GetVertices().end());
	m_indices.insert(m_indices.end(), p.GetIndices().begin(),
			 p.GetIndices().end());
}
...
{
	{
		...
		context->Map(m_vertexBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD,
			     0, &mappedResource);
		memcpy(mappedResource.pData, &m_vertices[0],
		       sizeof(Particle::Vertex) * m_vertices.size());
		context->Unmap(m_vertexBuffer.Get(), 0);
	}
	{
		D3D11_MAPPED_SUBRESOURCE mappedResource = {};
		context->Map(m_indexBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0,
			     &mappedResource);
		memcpy(mappedResource.pData, &m_indices[0],
		       sizeof(uint32_t) * m_indices.size());
		context->Unmap(m_indexBuffer.Get(), 0);
	}
}

Particle::Particle(ParticleType type, const Vec3D &accel, const Vec3D &initVel,
...
void Particle::CreateQuad(float width, float height, const Vec3D &up,
			  const Vec3D &right)
{
	Vec3D R = right;
...
{
	m_pos = 0.5f * m_lifespan * m_lifespan * m_accel +
		m_lifespan * m_initVel + m_initPos;
