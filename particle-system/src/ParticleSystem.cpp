...
ParticleSystem::ParticleSystem (const std::string &name, const Vec3D &origin,
                                const Camera &camera)
    : m_name{ name }, m_vertices{}, m_indices{}, m_origin{ origin },
      m_camera{ &camera }, m_emitter{
        ParticleType::Emitter, {}, {}, {}, 0.0f, *this
      }
{
...
void
ParticleSystem::Init (ID3D11Device *device, ID3D11DeviceContext *context,
                               ...
                               const std::string &filepath)
{
  ...
  ComPtr<ID3D11Texture2D> texture;
  {
    ...
    context->UpdateSubresource (
        texture.Get (), 0, nullptr, image.GetBytes (),
...
{
  {
    ...
    context->Map (m_vertexBuffer.Get (), 0, D3D11_MAP_WRITE_DISCARD, 0,
                  &mappedResource);
...
Particle::Particle (ParticleType type, const Vec3D &accel,
                    const Vec3D &initVel, const Vec3D &initPos,
                    const float lifespan, const ParticleSystem &ps)
    : m_accel{ accel }, m_initVel{ initVel }, m_initPos{ initPos }, m_pos{},
      m_lifespan{ lifespan }, m_type{ type }, m_particleSystem{ &ps }
{
}
...
void
Particle::CreateQuad (float width, float height, const Vec3D &up,
                      const Vec3D &right)
{
  Vec3D R = right;
