...
void CubeMap::CreateCube(const Actor& actor, ID3D11Device* device)
{
	...
	for (auto it = std::begin(actor.GetVertices()); it != std::end(actor.GetVertices()); ++it)
	{
		m_vertices.emplace_back(it->Position);
	}
	m_indices.insert(std::end(m_indices), std::begin(actor.GetIndices()), std::end(actor.GetIndices()));
	
	UtilsCreateVertexBuffer(device, &m_vertices[0], m_vertices.size(), sizeof(Vec3D), m_vertexBuffer.ReleaseAndGetAddressOf());
