...
// next iteration: skip loading a new texture 		bool skip = false;
// for (UINT j = 0; j < textures_loaded_.size(); j++) { if
// (std::strcmp(textures_loaded_[j].path.c_str(), str.C_Str()) == 0) {
// 				textures.push_back(textures_loaded_[j]);
// 				skip = true; // A texture with the same filepath
// has already been loaded, continue to next one. (optimization)
// break;
// 			}
// 		}
...
// scene->GetEmbeddedTexture(str.C_Str()); 			if
// (embeddedTexture
// != nullptr) { 				texture.texture =
// loadEmbeddedTexture(embeddedTexture); } else {
// std::string filename = std::string(str.C_Str());
// filename = directory_ + '/' + filename;
// std::wstring filenamews = std::wstring(filename.begin(), filename.end());
// hr = CreateWICTextureFromFile(dev_, devcon_, filenamews.c_str(), nullptr,
// &texture.texture); 				if (FAILED(hr))
// MessageBox(hwnd_, "Texture couldn't be loaded", "Error!", MB_ICONERROR |
// MB_OK);
// 			}
// 			texture.type = typeName;
// 			texture.path = str.C_Str();
// 			textures.push_back(texture);
// 			this->textures_loaded_.push_back(texture);  // Store it
// as texture loaded for entire model, to ensure we won't unnecesery load
// duplicate textures.
// 		}
// 	}
// 	return textures;
...
// ID3D11ShaderResourceView * ModelLoader::loadEmbeddedTexture(const aiTexture*
// embeddedTexture) { 	HRESULT hr; 	ID3D11ShaderResourceView *texture =
// nullptr;
...
// 		subresourceData.SysMemSlicePitch = embeddedTexture->mWidth *
// embeddedTexture->mHeight * 4;
...
// 		hr = dev_->CreateShaderResourceView(texture2D, nullptr,
// &texture); 		if (FAILED(hr)) MessageBox(hwnd_,
// "CreateShaderResourceView failed!", "Error!", MB_ICONERROR | MB_OK);
...
// 	hr = CreateWICTextureFromMemory(dev_, devcon_, reinterpret_cast<const
// unsigned char*>(embeddedTexture->pcData), size, nullptr, &texture); 	if
// (FAILED(hr)) 		MessageBox(hwnd_, "Texture couldn't be created
