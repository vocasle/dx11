...
void ShadowMap::InitResources(ID3D11Device *device, uint32_t texWidth,
			      uint32_t texHeight)
{
...
{
	ctx->OMGetRenderTargets(1, m_cachedRTV.ReleaseAndGetAddressOf(),
				m_cachedDSV.ReleaseAndGetAddressOf());
	uint32_t numViewports = 1;
	ctx->RSGetViewports(&numViewports, &m_cachedViewPort);

	ctx->ClearDepthStencilView(m_pOutputTextureDSV.Get(), D3D11_CLEAR_DEPTH,
				   1.0f, 0);
...
void ShadowMap::Unbind(ID3D11DeviceContext *ctx)
{
	...
	ctx->OMSetRenderTargets(1, m_cachedRTV.GetAddressOf(),
				m_cachedDSV.Get());
