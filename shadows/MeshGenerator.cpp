  ...
  for (size_t face = 0; face < indices.size () / 3; ++face)
    {
      ...
      if (i0 == uint32_t (-1) || i1 == uint32_t (-1) || i2 == uint32_t (-1))
        continue;
  ...
  for (size_t j = 0; j < vertices.size (); ++j)
    {
      ...
      Vec3D b1 = MathVec3DSubtraction (
          tan1, MathVec3DModulateByScalar (&b0, MathVec3DDot (&b0, &tan1)));
      MathVec3DNormalize (&b1);

      const Vec3D tan2 = tangent2[j];
      Vec3D b2 = MathVec3DSubtraction (
          MathVec3DSubtraction (tan2, MathVec3DModulateByScalar (
                                          &b0, MathVec3DDot (&b0, &tan2))),
          MathVec3DModulateByScalar (&b1, MathVec3DDot (&b1, &tan2)));
