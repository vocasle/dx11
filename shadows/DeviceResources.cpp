...
void DeviceResources::SetWindow(HWND hWnd, uint32_t width, uint32_t height)
{
	m_hWnd = hWnd;
...
#endif
	if (FAILED(CreateDXGIFactory2(dxgiFactoryFlags, IID_PPV_ARGS(m_Factory.ReleaseAndGetAddressOf()))))
	{
...
void DeviceResources::CreateDeviceResources()
{
	...
	if (FAILED(m_Factory->EnumAdapters(0, adapter.ReleaseAndGetAddressOf())))
	{
...
void DeviceResources::CreateWindowSizeDependentResources()
{
	...
	const UINT backBufferWidth = std::max(m_OutputSize.right - m_OutputSize.left, 1l);
	...
	else
	{
		...
		if (FAILED(m_Factory->CreateSwapChainForHwnd(m_Device.Get(),
			m_hWnd,
			&swapChainDesc,
			&fsSwapChainDesc,
			NULL,
		...
		if (FAILED(m_Factory->MakeWindowAssociation(m_hWnd, DXGI_MWA_NO_ALT_ENTER)))
		{
	...
	if (FAILED(m_SwapChain->GetBuffer(0, IID_PPV_ARGS(m_RenderTarget.ReleaseAndGetAddressOf()))))
	{
...

	{
		...
		if (FAILED(m_Device->CreateRenderTargetView(m_RenderTarget.Get(), &renderTargetViewDesc, m_RenderTargetView.ReleaseAndGetAddressOf())))
		{
...

	{
		...
		if (FAILED(m_Device->CreateTexture2D(&depthStencilDesc, NULL, m_DepthStencil.ReleaseAndGetAddressOf())))
		{
		...
		if (FAILED(m_Device->CreateDepthStencilView(m_DepthStencil.Get(), &depthStencilViewDesc, m_DepthStencilView.ReleaseAndGetAddressOf())))
		{
