...
{
  MeshInfo ()
      : NumPositions{ 0 }, NumNormals{ 0 }, NumTexCoords{ 0 }, NumFaces{ 0 }
  {
  }
...
static void
OLParseMeshes (std::vector<Mesh> &meshes, std::vector<MeshInfo> &infos,
               FILE *objfile)
{
  char line[128];
  char prefix[4];
  ...
  while (OLReadLine (objfile, line, 128))
    {
      ...
      if (strcmp (prefix, "o ") == 0)
        ...
        {
          result = sscanf_s (line, "%s%d/%d/%d%d/%d/%d%d/%d/%d", prefix, 4,
                             idx, idx + 1, idx + 2, idx + 3, idx + 4, idx + 5,
                             idx + 6, idx + 7, idx + 8);
          assert (result == 10);
          OLLogInfo ("Face { %d/%d/%d %d/%d/%d %d/%d/%d }", idx[0], idx[1],
                     idx[2], idx[3], idx[4], idx[5], idx[6], idx[7], idx[8]);
