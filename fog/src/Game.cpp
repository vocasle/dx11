  ...
  if (ImGui::CollapsingHeader ("Fog settings"))
    {
      ...
      ImGui::ColorPicker4 (
          "Fog color", reinterpret_cast<float *> (&m_PerSceneData.fogColor));
    }
...
Game::Update ()
{
  ...
  g_FogCBuf->SetValue ("fogColor",
                       *reinterpret_cast<Vec4D *> (&m_PerSceneData.fogColor));
...
Game::Render ()
{
  ...
  m_DR->PIXBeginEvent (L"Fog");
  {
    ...
    m_Renderer.BindShaderResource (BindTargets::PixelShader,
                                   g_OffscreenRTV->GetDepthSRV (), 1);
    m_Renderer.BindConstantBuffer (BindTargets::VertexShader,
                                   g_FogCBuf->Get (), 0);
    m_Renderer.BindConstantBuffer (BindTargets::PixelShader, g_FogCBuf->Get (),
                                   0);

    const auto fogPlane = FindActorByName ("FogPlane");
    m_Renderer.SetVertexBuffer (fogPlane->GetVertexBuffer (), sizeof (Vertex),
                                0);
...
Game::Initialize (HWND hWnd, uint32_t width, uint32_t height)
{
  ...
  g_OffscreenRTV = std::make_unique<Texture> (
      DXGI_FORMAT_B8G8R8A8_UNORM, m_DR->GetOutputSize ().right,
      m_DR->GetOutputSize ().bottom, m_DR->GetDevice ());
