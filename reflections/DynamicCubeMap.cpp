...
void DynamicCubeMap::BuildCubeFaceCamera(const Vec3D& origin)
{
...
void DynamicCubeMap::CreateRenderTargets(ID3D11Device* device)
{
	//
...
void DynamicCubeMap::CreateDepthBuffer(ID3D11Device* device)
{
	...
	HR(device->CreateDepthStencilView(depthTex.Get(),
		&dsvDesc, m_dynamicCubeMapDSV.ReleaseAndGetAddressOf()));
}

void DynamicCubeMap::CreateViewport(ID3D11Device* device)
{
