...
Mat4X4
MathMat4X4Orthographic (float viewWidth, float viewHeight, float zNear,
                        float zFar)
{
...
Mat4X4
MathMat4X4ViewAt (const Vec3D *eyePos, const Vec3D *focusPos,
...
Mat4X4
MathMat4X4PerspectiveFov (float fovAngleY, float aspectRatio, float nearZ,
                          float farZ)
{
  Mat4X4 res = {};
...
Mat4X4
MathMat4X4OrthographicOffCenter (float viewLeft, float viewRight,
                                 float viewBottom, float viewTop, float nearZ,
                                 float farZ)
{
...
MathMat4X4Inverse (const Mat4X4 *mat)
{
  ...
  const XMFLOAT4X4 xmmat
      = XMFLOAT4X4{ static_cast<const float *> (&mat->A00) };
  XMFLOAT4X4 xminverse;
  XMStoreFloat4x4 (&xminverse,
                   XMMatrixInverse (nullptr, XMLoadFloat4x4 (&xmmat)));
...
Vec4D
MathVec4DVectorPermute (Vec4D V1, Vec4D V2, uint32_t PermuteX,
...
TestVec3D (void)
{
  ...
  assert (MathNearlyEqual (vec1.X, -10.0f)
          && MathNearlyEqual (vec1.Y, -10.0f));
