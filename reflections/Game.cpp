...
void
Game::CreatePixelShader (const char *filepath, ID3D11Device *device,
      ...
      if (actor.IsVisible ())
        {
          ...
          m_Renderer.SetVertexBuffer (
              ...
              m_InputLayout.GetVertexSize (InputLayout::VertexType::Default),
              0);
...
Game::DrawSky ()
{
  ...
  m_Renderer.SetVertexBuffer (
      m_CubeMap.GetVertexBuffer (),
      m_InputLayout.GetVertexSize (InputLayout::VertexType::Sky), 0);
  m_Renderer.BindConstantBuffer (BindTargets::VertexShader,
                                 m_PerObjectCB.Get (), 0);
  m_Renderer.BindConstantBuffer (BindTargets::VertexShader,
                                 m_PerFrameCB.Get (), 1);
...
std::vector<uint8_t>
Game::CreateVertexShader (const char *filepath, ID3D11Device *device,
...
Game::Update ()
{
  ...
  Vec3D translated
      = { cosf (totalTime) * radius, 1.0f, sinf (totalTime) * radius };
...
Game::Render ()
{
  ...
  // for (uint32_t i = 0; i < _countof(m_PerSceneData.pointLights); ++i)
  {
    ...
    m_Renderer.SetVertexBuffer (
        ...
        m_InputLayout.GetVertexSize (InputLayout::VertexType::Default), 0);
...
Game::CreateActors ()
{
  ...
  {
    const Vec3D positions[] = {
      { -4.0f, 0.0f, 4.0f }, { -4.0f, 0.0f, 0.0f }, { -4.0f, 0.0f, -4.0f },
      { 4.0f, 0.0f, 4.0f },  { 4.0f, 0.0f, 0.0f },  { 4.0f, 0.0f, -4.0f },
    };

    Actor actor = Actor ();
    actor.LoadModel (
        UtilsFormatStr ("%s/meshes/cylinder.obj", ASSETS_ROOT).c_str ());
    actor.CreateIndexBuffer (m_DR->GetDevice ());
    actor.CreateVertexBuffer (m_DR->GetDevice ());
    Material material ({ 0.23125f, 0.23125f, 0.23125f, 1.0f },
                       { 0.2775f, 0.2775f, 0.2775f, 1.0f },
    ...
    actor.SetMaterial (&material);
    actor.LoadTexture (
        UtilsFormatStr ("%s/textures/marble_diffuseOriginal.jpg", ASSETS_ROOT)
            .c_str (),
        TextureType::Diffuse, m_DR->GetDevice (), m_DR->GetDeviceContext ());
    actor.LoadTexture (
        UtilsFormatStr ("%s/textures/marble_normal.jpg", ASSETS_ROOT).c_str (),
        TextureType::Normal, m_DR->GetDevice (), m_DR->GetDeviceContext ());
    actor.LoadTexture (
        UtilsFormatStr ("%s/textures/marble_smoothness.jpg", ASSETS_ROOT)
            .c_str (),
        TextureType::Gloss, m_DR->GetDevice (), m_DR->GetDeviceContext ());
    actor.LoadTexture (
        UtilsFormatStr ("%s/textures/marble_metallic.jpg", ASSETS_ROOT)
            .c_str (),
        TextureType::Specular, m_DR->GetDevice (), m_DR->GetDeviceContext ());
    actor.SetIsVisible (true);
...
Game::Initialize (HWND hWnd, uint32_t width, uint32_t height)
{
  ...
  bytes = CreateVertexShader ("SkyVS.cso", device,
                              m_SkyVS.ReleaseAndGetAddressOf ());
...
Game::BuildShadowTransform ()
{
  ...
  // Vec3D lightPos = MathVec3DModulateByScalar(&lightDir,
  // m_sceneBounds.Radius); Vec3D targetPos = { 0.0f, 0.0f, 0.0f }; Vec3D up =
  // { 0.0f, 1.0f, 0.0f };
  ...
  // GameUpdateConstantBuffer(m_DR->GetDeviceContext(),
  // sizeof(PerFrameConstants), &m_PerFrameData, m_PerFrameCB.Get());
}