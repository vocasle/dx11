...
VSOut main(VSIn In)
{
	...
	Out.PosW = mul(worldInvTranspose, float4(In.Pos, 1.0f)).xyz;
	Out.ShadowPosH = mul(shadowTransform, float4(In.Pos, 1.0f));
	Out.TangentW = mul(worldInvTranspose, float4(In.Tangent, 0.0f));
	Out.BitangentW = mul(worldInvTranspose, float4(In.Bitangent, 0.0f));
	return Out;
}
